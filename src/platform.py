...
class platform:
    ...
    def __str__(self):
        return '\n'.join(str(reel) for reel in self.reels)
    ...
    def generate_platform(self, user_count: int = 100, reel_count: int = 1000, peruser: int = 10, devcount: int = 40,
                          tagcount: int = 3, friend_count: int = 10):
        mean = peruser
        standard_deviation = 3
        user_normal_integers = generate_normal_integers(mean, standard_deviation, devcount)
        friend_normal_integers = generate_normal_integers(friend_count, 4, devcount)
        ...
        for i in range(reel_count):
            ...
            t = random.sample(self.tags, n)
            length = random.sample(length_normal_integers, 1)
            r = self.Reel(i + 1, t, length)
            self.reels.append(r)
        ...
        for i in range(user_count):
            ...
            reels = random.sample(self.reels, n)
            # Choose f random friends
            friends = random.sample(full_user_ids, f)
            number_likes = random.randint(0, n)
            number_dislikes = random.randint(0, n - number_likes)
